...
export const metadata = {
  ...
  description:
    "台灣第十一屆立法委員依持股市值排名，查看哪位立委持有最多股票資產。",
}
  ...
  const ranked = declarations
    .map((d) => {
      ...
      return {
        ...
        stockCount:
          d.securities.stocks.items.length + d.securities.funds.items.length,
      }
    })
    ...
    <div className="space-y-8">
      ...
      <JsonLd
        data={createBreadcrumbList([
          ...
          { name: "持股市值排行榜", path: "/legislator/rankings" },
        ])}
      />
      ...
      <header className="pt-4">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {ranked.length}{" "}
          位立法委員依持股市值由高至低排列，市值依據台灣證交所收盤價估算，並每日更新。
        </p>
        <LegislatorNav current="rankings" className="mt-5" />
      </header>
