...
function getTopHolding(
  data: LegislatorDeclaration
): { name: string; code?: string; value: number } | null {
...
export default async function LegislatorPage({
  ...
  const entry = getLegislatorIndexEntryBySlug(slug)
  ...
  const partyDot = meta?.party
    ? PARTY_COLOR[meta.party] || "bg-muted-foreground"
    : ""
  const marketTotal = calcMarketTotal(data)
  const stockCount =
    data.securities.stocks.items.length + data.securities.funds.items.length
  const topHolding = getTopHolding(data)
  const downloads = getDeclarationPdfDownloads("legislators", [
    ...(entry?.declarations[0] ? [entry.declarations[0]] : []),
    ...(entry?.changes ?? []),
  ])
  ...
  const samePartyLegislators = meta?.party
    ? allDeclarations
        .filter(
          (d) =>
            d.name !== data.name &&
            getLegislatorMeta(d.name)?.party === meta.party
        )
        .slice(0, 8)
    : []
    ...
    <div className="space-y-8">
      <JsonLd
        data={{
          ...
          "@type": "Person",
          name: data.name,
          url: pageUrl(`/legislator/${slug}`),
          ...(meta?.avatar ? { image: assetUrl(meta.avatar) } : {}),
          ...(meta?.party
            ? { memberOf: { "@type": "Organization", name: meta.party } }
            : {}),
      ...
      <JsonLd
        data={createBreadcrumbList([
          ...
          { name: data.name, path: `/legislator/${slug}` },
        ])}
      />
      ...
      <div className="flex items-start gap-4 pt-2">
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
            ...
            {data.spouse && <span>配偶：{data.spouse.name}</span>}
          </div>
        </div>
      </div>
      ...
      <p className="text-sm leading-relaxed text-muted-foreground">
        {data.name}為{meta?.party ? `${meta.party}籍` : ""}第十一屆立法委員，依{" "}
        {formatDate(data.declarationDate)} 監察院財產申報資料，共持有{" "}
        {stockCount} 檔有價證券，總市值約 NT$ {formatNTD(marketTotal)}
        {topHolding
          ? `，最大持股為${topHolding.name}${topHolding.code ? `（${topHolding.code}）` : ""}`
          : ""}
        。
      </p>
          ...
          <div className="flex flex-wrap gap-2">
            {samePartyLegislators.map((d) => (
              <Link
                ...
                className="inline-flex items-center gap-1 rounded-md border px-2.5 py-1 text-sm transition-colors hover:bg-muted"
              >
                {d.name}
              </Link>
            ))}
