...
export default function StocksPage() {
  ...
  const withHoldings = declarations.filter(
    (d) =>
      d.securities.stocks.items.length + d.securities.funds.items.length > 0
  )
  ...
  const allStocksWithDetails = aggregatedStocks.map((s) => {
    ...
    for (const h of s.holders) {
      ...
      if (existing) {
        ...
        uniqueHolders.set(h.legislator, {
          shares: h.shares,
          ntdTotal: h.ntdTotal,
        })
      }
    ...
    const holderDetails = Array.from(uniqueHolders.entries())
      .map(([legislator, { shares, ntdTotal }]) => {
    ...
    <div className="space-y-16">
      ...
      <JsonLd
        data={createBreadcrumbList([
          ...
          { name: "股票及基金", path: "/legislator/stocks" },
        ])}
      />
      <header className="pt-4">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {declarations.length} 位立委中有 {withHoldings.length}{" "}
          位持有股票或基金，共 {new Set(holdings.map((h) => h.name)).size}{" "}
          檔標的、{holdings.length} 筆紀錄。
        </p>
        <LegislatorNav current="stocks" className="mt-5" />
      </header>
      ...
      {concentratedRows.length > 0 && (
        <section className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            單一股票佔該立委持股 20% 以上，且市值超過 500 萬
          </p>
          <div className="divide-y">
            {concentratedRows.map((r, i) => (
              <div
                key={`${r.legislator}-${r.stock}-${i}`}
