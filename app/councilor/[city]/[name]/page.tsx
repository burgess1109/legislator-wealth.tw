...
export async function generateStaticParams() {
  ...
  for (const councilor of getCouncilorIndex().councilors) {
    if (
      councilor.declarations.length === 0 ||
      !getCouncilorDeclarationBySlug(councilor.slug)
    )
      continue
...
export async function generateMetadata({
  ...
  if (!cityName || !data) return { title: memberSlug }
  ...
  const url = `/councilor/${citySlug}/${canonicalName}/`
  const image = `/og/councilors/${slug}.png`
...
export default async function CouncilorDetailPage({
  params,
}: {
  params: Promise<{ city: string; name: string }>
}) {
  ...
  const canonicalMemberSlug = getCouncilorMemberSlug(slug, citySlug)
  ...
  const stockCount = data
    ...
    : 0
  const downloads = getDeclarationPdfDownloads("councilors", [
    ...
    <div className="space-y-8">
      <JsonLd
        data={{
          ...
          "@type": "Person",
          name: displayName,
          url: pageUrl(`/councilor/${citySlug}/${canonicalMemberSlug}`),
          ...(avatar ? { image: assetUrl(avatar) } : {}),
          ...(meta?.party
      ...
      <JsonLd
        data={createBreadcrumbList([
          ...
          {
            name: displayName,
            path: `/councilor/${citySlug}/${canonicalMemberSlug}`,
          },
        ])}
      />
      ...
      <div className="flex items-start gap-4 pt-2">
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
            <span>
              {meta?.organization ?? data?.organization ?? entry?.organization}
            </span>
      ...
      {data ? (
        <>
          <p className="text-sm leading-relaxed text-muted-foreground">
            {displayName}為{meta?.party ? `${meta.party}籍` : ""}
            {cityName}議員，依 {formatDate(data.declarationDate)}{" "}
        ...
        <section className="border-y py-8">
          ...
          {meta?.detailUrl && (
            <a
              ...
              className="mt-3 inline-flex text-sm underline hover:text-foreground"
            >
              內政部議員資料
            </a>
          )}
