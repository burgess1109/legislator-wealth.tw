...
export async function generateMetadata({
  ...
  const title = cityName ? `${cityName}議員持股總覽` : "地方議員持股總覽"
  const description = cityName
    ? `${cityName}議員申報的股票與基金持有明細。`
    : "地方議員申報的股票與基金持有明細。"
  const url = `/councilor/${citySlug}/stocks/`
...
export default async function CouncilorStocksPage({
  params,
}: {
  params: Promise<{ city: string }>
}) {
  ...
  const allStocksWithDetails = aggregatedStocks.map((stock) => {
    ...
    for (const holder of stock.holders) {
      ...
      if (existing) {
        ...
        uniqueHolders.set(holder.legislator, {
          shares: holder.shares,
          ntdTotal: holder.ntdTotal,
        })
      }
    }

    let marketValue = 0
    const holderDetails = Array.from(uniqueHolders.entries())
      .map(([councilor, { shares, ntdTotal }]) => {
  ...
  const concentratedRows = allStocksWithDetails
    .flatMap((stock) =>
      stock.holderDetails
        .filter(
          (holder) => holder.pctOfPortfolio >= 20 && holder.value >= 5_000_000
        )
        .map((holder) => ({
          stock: stock.name,
    ...
    <div className="space-y-12">
      ...
      <JsonLd
        data={createBreadcrumbList([
          ...
          { name: "股票及基金", path: `/councilor/${citySlug}/stocks` },
        ])}
      />
      ...
      <header className="space-y-3 pt-4">
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {rows.length} 位議員中有 {withHoldings.length} 位持有股票或基金，共{" "}
            {new Set(holdings.map((h) => h.name)).size} 檔標的、
            {holdings.length} 筆紀錄。
          </p>
        </div>
      ...
      {concentratedRows.length > 0 && (
        <section className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            單一股票佔該議員持股 20% 以上，且市值超過 500 萬
          </p>
          <div className="divide-y">
            {concentratedRows.map((row, index) => (
              <div
                key={`${row.councilor}-${row.stock}-${index}`}
                className="flex items-center gap-3 py-2"
              >
                <span className="w-10 shrink-0 font-heading text-sm font-black text-[#cc4444] tabular-nums">
                  {row.pct}%
                </span>
                {row.href ? (
                  <Link
