...
export async function generateMetadata({
  ...
  const title = `${cityName}議員財產申報 — 地方議員持股排行`
  const description = `${cityName}議員名單、財產申報持股市值排行、黨籍整理與個別議員持股資料。`
  const ogTitle = `${cityName}議員財產申報`
  const ogDescription = `${cityName}議員財產申報持股資料與市值排行。`
  const url = `/councilor/${citySlug}/`
  ...
  return {
    ...
    openGraph: {
      ...
      url,
      images: [createOgImage(image, ogTitle)],
    },
    twitter: {
...
function buildTopHoldings(rows: CouncilorListItem[]) {
  ...
  for (const row of rows) {
    ...
    for (const stock of declaration.securities.stocks.items) {
      ...
      const value = price
        ? Math.round(stock.shares * price.price)
        : stock.ntdTotal
      const existing = holdings.get(stock.name) ?? {
        name: stock.name,
        holderNames: new Set<string>(),
        total: 0,
      }
    ...
    for (const fund of declaration.securities.funds.items) {
      ...
      const existing = holdings.get(fund.name) ?? {
        name: fund.name,
        holderNames: new Set<string>(),
        total: 0,
      }
  ...
  return Array.from(holdings.values())
    .map((holding) => ({
      ...
      holders: Array.from(holding.holderNames).sort((a, b) =>
...
export default async function CouncilorCityPage({
  ...
  const { city: citySlug } = await params
  const cityName = getCouncilorCityName(citySlug)
  if (!cityName || !hasCouncilorCityDeclarations(citySlug)) {
    notFound()
  }
    ...
    <div className="space-y-10">
      ...
      <header className="space-y-3 pt-4">
        ...
        <p className="max-w-3xl text-sm leading-relaxed text-muted-foreground">
          名單取自內政部地方公職人員資訊專區，申報 PDF
          解析後會在此頁提供市值排行、黨籍整理、股票與基金統計。
        </p>
        {source.fetchedAt && (
      ...
      <section className="space-y-3">
        <div className="flex items-baseline justify-between">
          ...
          <Link
            href={`/councilor/${citySlug}/rankings`}
        ...
        {leader ? (
          <div className="space-y-4">
            <Link href={leader.href} className="group block border-b pb-8">
              ...
              <div className="flex items-end gap-5 sm:gap-8">
                <div
                  className={`flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden border-l-4 bg-muted text-4xl font-black text-muted-foreground ${leader.party ? PARTY_BORDER[leader.party] || "" : ""} sm:h-32 sm:w-32`}
                >
                  {leader.avatar ? (
                    <img
                      src={leader.avatar}
                ...
                <div className="min-w-0 flex-1 space-y-1">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {leader.party || "未標示黨籍"} · {leader.stockCount} 檔
                  </p>
                ...
                {ranked.slice(1, 10).map((row) => {
                  ...
                  return (
                    <Link
                      key={row.slug}
                      href={row.href}
                      className="row-hover relative grid grid-cols-[2.5rem_2.5rem_minmax(0,1fr)_auto] items-center gap-3 px-3 py-2.5 hover:bg-muted/50"
                    >
                      <div
                      ...
                      <div
                        className={`relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden border-l-2 bg-muted text-sm font-bold text-muted-foreground ${row.party ? PARTY_BORDER[row.party] || "" : ""}`}
                      >
                        {row.avatar ? (
                          <img
                            src={row.avatar}
                      ...
                      <span className="relative shrink-0 font-bold tracking-tight tabular-nums">
          ...
          <div className="border-y py-8">
            <p className="text-sm text-muted-foreground">
              尚未建立持股排行榜。把{cityName}議員申報 PDF 放進
              `raw-pdfs/councilors/` 並重新解析後，這裡會自動顯示排名。
            </p>
          </div>
        )}
      </section>
      ...
      {topHoldings.length > 0 && (
        <section className="space-y-3">
          <div className="flex items-baseline justify-between">
            ...
            <Link
              href={`/councilor/${citySlug}/stocks`}
              className="text-sm text-muted-foreground hover:text-foreground"
            >
              全部明細
            </Link>
          ...
          <div className="divide-y border-y">
            {topHoldings.map((holding, index) => (
              <div
                key={holding.name}
                className="grid gap-2 px-3 py-3 sm:grid-cols-[2.5rem_minmax(0,1fr)_auto] sm:items-center"
              >
                <span className="hidden text-right text-lg font-black text-muted-foreground/25 tabular-nums sm:block">
                  #{index + 1}
                </span>
                <div className="min-w-0">
                  <p className="truncate font-bold">{holding.name}</p>
                  <p className="truncate text-xs text-muted-foreground">
                    {holding.holders.join("、")}
                  </p>
                </div>
                <div className="text-sm sm:text-right">
                  <p className="font-bold tabular-nums">
                    {holding.holderCount} 位
                  </p>
                  <p className="text-xs text-muted-foreground">
                    <CurrencyDisplay amount={holding.total} />
                  </p>
                </div>
              </div>
            ))}
