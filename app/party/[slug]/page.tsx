    ...
    <div className="space-y-8">
      ...
      <JsonLd
        data={createBreadcrumbList([
          ...
          { name: partyName, path: `/party/${slug}` },
        ])}
      />
      ...
      <header className="pt-4">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          共 {partyLegislators.length} 位立委，持股總市值合計 NT${" "}
          {formatNTD(totalValue)}。
        </p>
      </header>
          ...
          {Object.entries(PARTY_SLUG_MAP)
            ...
            .map(([s, name]) => (
              <Link
                key={s}
                href={`/party/${s}`}
                className="inline-flex items-center rounded-md border px-3 py-1.5 text-sm transition-colors hover:bg-muted"
              >
                {name}
              </Link>
            ))}
