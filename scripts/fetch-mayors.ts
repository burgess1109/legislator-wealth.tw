...
async function fetchPage(source: MayorSource, page: number): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(`MOI mayor fetch failed for ${source.title} page ${page}: ${response.status}`)
  }
...
async function main() {
  ...
  const filtered = dedupeMayors(all)
    ...
    .sort((a, b) => a.city.localeCompare(b.city, 'zh-TW') || a.name.localeCompare(b.name, 'zh-TW'))
  const cities = Array.from(new Set(filtered.map(mayor => mayor.city)))
  ...
  for (const mayor of filtered) {
    ...
    const avatarDownloaded = await downloadImage(mayor.avatar, path.join(AVATAR_DIR, `${slug}.jpg`))
    slugCounts.set(mayor.slug, count + 1)
    bySlug[slug] = { ...mayor, slug, avatar: avatarDownloaded ? avatarPath : mayor.avatar }
  }
  ...
  console.log(`Fetched ${filtered.length} mayor(s) for ${cities.join(', ')} from MOI`)
